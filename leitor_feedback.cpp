//AVISOS: TALVEZ VERIFICAR ROTAS DE MQTT NOVAMENTE (DEPOIS DA OPCAO DO PREFERENCE, PODE TER QUEBRADO LOGICA)
//TALVEZ CONCATENAR OQ TA SALVO NO PREFERENCE COM "".../catraca/..." ONDE FOR NECESSARIO!
...
void reconectarMQTT() {
  ...
  if (mqtt.connected()){
    ...
    String MQTTtopic = preferencias.getString("mqttTopic", "");
...
void recebeuMensagem(String topic, String content) {
  ...
  String topicoResposta = preferencias.getString("mqttTopic", "");
  if (topic == topicoResposta) {                           //fiz uma troca possivelmente problematica aq (pegando do preferences o mqtt) 
...
void setup() {
  ...
  if (mqttHost.isEmpty()) {
    Serial.println("Broker MQTT não configurado.");
  }
  else {
      mqtt.begin(mqttHost.c_str(), mqttPort, conexaoSegura);
  }
