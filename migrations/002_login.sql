...
-- ENG4051 — PUC-Rio
--
-- Evolui o schema original (funcionarios.uid_cartao 1:1 + logs_acesso/autorizado)
-- para o novo modelo: cartões RFID separados do funcionário, permissões por área,
-- usuários/sessões para login e registros_acesso com motivo de negação detalhado.
...
-- Migra o uid_cartao que hoje vive em funcionarios para a nova tabela
INSERT INTO cartoes_rfid (uid, id_funcionario, ativo)
SELECT uid_cartao, id, ativo FROM funcionarios
ON CONFLICT (uid) DO NOTHING;

ALTER TABLE funcionarios DROP COLUMN IF EXISTS uid_cartao;

-- ── Permissões por área (cartão x área) ───────────────────────────────────
CREATE TABLE IF NOT EXISTS permissoes (
  id_cartao   INT NOT NULL REFERENCES cartoes_rfid(id) ON DELETE CASCADE,
  id_area     INT NOT NULL REFERENCES areas(id) ON DELETE CASCADE,
...
-- ── Registros de acesso (substitui logs_acesso, distingue tipos de negação) ─
...
SELECT create_hypertable('registros_acesso', 'data_hora', if_not_exists => TRUE);

-- Migra o histórico de logs_acesso (autorizado boolean) para o novo enum
INSERT INTO registros_acesso (uid, id_funcionario, resultado, data_hora)
SELECT
  l.uid_cartao,
  f.id,
  ...
  l.data_hora
FROM logs_acesso l
LEFT JOIN funcionarios f ON f.nome = l.nome_funcionario;

DROP TABLE IF EXISTS logs_acesso;

CREATE INDEX IF NOT EXISTS idx_registros_acesso_uid ON registros_acesso (uid, data_hora DESC);
CREATE INDEX IF NOT EXISTS idx_registros_acesso_funcionario ON registros_acesso (id_funcionario, data_hora DESC);
