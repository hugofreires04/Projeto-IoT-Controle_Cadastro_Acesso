// acessos.js — Componente de tabela de acessos, reutilizado em admin.html e
// operador.html (mesmo código, comportamentos diferentes via initTabelaAcessos).
//
...
//
// A restrição de visibilidade NÃO é feita aqui: para operadores o backend
// força o filtro pelo próprio id_funcionario (ver routes/acessos.py).

// Tradução dos códigos de resultado do banco para rótulo + cor do badge.
...
// Estado do componente: página atual, filtro de resultado ativo e se o
// filtro por funcionário existe nesta página (só no painel admin).
...
// Converte o estado dos filtros da tela em query string para a API.
// A mesma query serve para a listagem paginada e para o export CSV.
...
async function _carregarAcessos() {
    ...
    `).join("") || `<tr><td colspan="4" class="vazio">Nenhum registro encontrado.</td></tr>`;
...
// Baixa o CSV com os MESMOS filtros aplicados na tela (sem paginação).
// O download é disparado criando um link temporário para o blob da resposta.
...
// Inicializa o componente. mostrarFiltroFuncionario=true no painel admin
// (que pode filtrar por qualquer funcionário); false no painel do operador.
